...
func generate_maze(maze: Maze, is_generated_slowly: bool) -> void:
	...
	var maze_grid: Array = await generate_grid(maze.width, maze.height, maze.wall_length, maze.wall_height, maze.wall_thickness)
	
	var cell_stack: Array = []
	
	var initial_cell: Node3D = randomize_start_position(maze_grid, maze.width, maze.height)
	initial_cell.visit()
