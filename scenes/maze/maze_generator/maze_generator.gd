...
func center_maze_on_screen(maze: Maze) -> void:
	var maze_position_x: int = ((maze.width * maze.wall_length) - maze.wall_length)
	var maze_position_z: int = ((maze.height * maze.wall_length) - maze.wall_length)
...
func get_random_cell(maze_grid: Array[Array]) -> MazeCell:
	var cell_x_axis: int = rng.randi_range(0, maze_grid.size() - 1)
	var cell_y_axis: int = rng.randi_range(0, maze_grid[0].size() - 1)
